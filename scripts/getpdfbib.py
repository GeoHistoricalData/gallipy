...
parser = argparse.ArgumentParser(
    description="A simple script to download the PDF version of an archival resource hosted by Gallica."
)
parser.add_argument(
    "bibtex",
    type=str,
    ...
    for idx, bib in enumerate(bib_database.entries):
        ...
        logging.info(
            "Downloading {} {}: {} [{}]{}".format(
                ...
                bibkey,
                ark,
                ", views {} to {}".format(vues[0], vues[1]) if bib.get("vues") else "",
            )
        )
        name = "{}{}.pdf".format(
            bibkey, "_" + str(vues[0]) + "_" + str(vues[1]) if bib.get("vues") else ""
        )
