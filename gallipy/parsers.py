...
class PaginationParser:
    ...
    __structure_elements = ["firstDisplayedPage","hasToc","TocLocation","hasContent","idUPN","nbVueImages"]
    __page_elements = ["numero","ordre","pagination_type","image_width","image_height"]
    ...
    def structure(self):
        ...
        d = {k:cast_or(s.find(k).text, *cast_fb) for k in  PaginationParser.__structure_elements}
        return d

    def page_list(self):
        cast_fb = [int, bool,str]
        for p in self._parsed.find_all("page"):
            page = {k:cast_or(p.find(k).text, *cast_fb) for k in  PaginationParser.__page_elements}
            yield page
...
def html2text(bs_html):
    ...
    text = [node if isinstance(node, NavigableString) else node.get_text() for node in bs_html]
    return "".join(text)
